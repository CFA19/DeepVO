...
#from DeepVO.deepvonet import DeepVONet, FlowNet
...
#    val_dataset = VisualOdometryDataLoader(config['datapath'], 192, 640, config['bsize'], val=True)
    train_dataset = VisualOdometryDataLoader(config['datapath'], 375, 1242, config['bsize'])
    val_dataset = VisualOdometryDataLoader(config['datapath'], 375, 1242, config['bsize'], val=True) 


    criterion = tf.keras.losses.MeanSquaredError()
    #optimizer = None
    if config['train'] == 'deepvo':
        optimizer = tf.keras.optimizers.SGD(config['lr'], 0.99, True)
...
def main():
    ...
    config = {
        #'datapath': '/media/cfa/CFA4TB/VIT/PhD/Research/PhDTest/Datasets/datasets/datasetrgb/dataset',
        'datapath': 'E:\VIT\PhD\Research\PhDTest\Datasets\datasets\datasetrgb\dataset',
        'bsize': 16,
        'lr': 0.001,
        #'momentum': 0.99,
        # 'lr': 0.0001,
        'train_iter': 20,
        'checkpoint_path': "E:\\VIT\\PhD\\Research\\PhDTest\\2023\\working\\VisualOdometry\\checkpoints",
        #'checkpoint_path':'/media/cfa/CFA4TB/VIT/PhD/Research/PhDTest/2023/working/VisualOdometry-master/checkpoints',
        'k': 100,
        'train': 'deepvo'
    }
